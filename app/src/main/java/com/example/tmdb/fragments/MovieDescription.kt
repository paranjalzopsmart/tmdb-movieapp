...
class MovieDescription(val MovieDataObject: MovieData) : Fragment() {
    ...
    override fun onCreateView(
       ...
       binding= FragmentMovieDescriptionBinding.inflate(layoutInflater)
        ...
        context.let {
            if (it != null) {
                Glide.with(it).load(Base_URL+ MovieDataObject.poster_path).into(binding.moviePoster)
            }
        }
        context.let {
            if (it != null) {
                Glide.with(it).load(Base_URL+ MovieDataObject.backdrop_path).into(binding.movieBackdrop)
            }
        }
